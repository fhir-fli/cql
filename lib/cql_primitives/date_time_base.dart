...
abstract class CqlDateTimeBase extends CqlPrimitive
  ...
  bool? _compare(Comparator comparator, Object o) {
    ...
    rhs = rhs == null
        ...
        : CqlDateTime.fromUnits(
            ...
            microsecond: rhs.microsecond == null
                ? null
                : int.tryParse(rhs.microsecond!),
          );
  ...
  static int? _compareWithPrecision(
      CqlDateTimeBase lhs, CqlDateTimeBase rhs) {
  ...
  static CqlDateTimeBase plus<T>(CqlDateTimeBase base, ExtendedDuration o) {
    final micros =
        int.tryParse(base.microsecond?.padRight(6, '0') ?? '0') ?? 0;
  ...
  bool? isBefore(CqlDateTimeBase other) =>
  ...
  bool? isEquivalent(Object other) =>
      _compare(Comparator.equivalent, other);
  ...
  static Map<String, dynamic> formatDateTimeString<T>(String s) {
    ...
    return <String, Object?>{
      ...
      'minute': T == CqlDate
          ? null
          : int.tryParse(match.namedGroup('minute') ?? ''),
      'second': T == CqlDate
          ? null
          : int.tryParse(match.namedGroup('second') ?? ''),
...
enum TemporalPrecisionEnum {
  ...
  DateTime add(DateTime input, int amount) {
    switch (this) {
      case TemporalPrecisionEnum.year:
        return DateTime(input.year + amount, input.month, input.day,
            input.hour, input.minute, input.second, input.millisecond);
      case TemporalPrecisionEnum.month:
        return DateTime(input.year, input.month + amount, input.day,
            input.hour, input.minute, input.second, input.millisecond);
      case TemporalPrecisionEnum.day:
        return DateTime(input.year, input.month, input.day + amount,
            input.hour, input.minute, input.second, input.millisecond);
      case TemporalPrecisionEnum.hour:
